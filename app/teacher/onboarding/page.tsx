...
import { z } from "zod"
import { GraduationCap, Briefcase, BookOpen, DollarSign, Calendar, User, ArrowRight, ArrowLeft, Plus, Trash2, CheckCircle } from "lucide-react"
        ...
        <JarvisCard glow="violet" className="p-8">
          <div className="mb-6 flex items-center justify-between">
            {STEPS.map((s, i) => (
              <div key={s.label} className="flex items-center">
                <div className={`flex h-8 w-8 items-center justify-center rounded-full border text-xs font-bold ${i < step ? "border-accent-success bg-accent-success/20 text-accent-success" : i === step ? "border-accent-primary bg-accent-primary/20 text-accent-primary" : "border-border text-text-disabled"}`}>
                  {i < step ? <CheckCircle className="h-4 w-4" /> : i + 1}
                </div>
                {i < STEPS.length - 1 && <div className={`mx-1 h-px w-4 sm:w-8 ${i < step ? "bg-accent-success" : "bg-border"}`} />}
              </div>
            ))}
          </div>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
              <h3 className="mb-4 font-display text-lg font-bold">{STEPS[step].label}</h3>

              {step === 0 && (
                <div className="space-y-4">
                  <JarvisInput label="Display Name" placeholder="Your teaching name" icon={<User className="h-4 w-4" />} error={form.formState.errors.display_name?.message} {...form.register("display_name")} />
                  <JarvisInput label="Tagline" placeholder="e.g. Physics PhD | 10+ years" icon={<GraduationCap className="h-4 w-4" />} {...form.register("tagline")} />
                  <div>
                    ...
                    {form.watch("languages").map((_: string, i: number) => (
                      <div key={i} className="mb-2 flex gap-2">
                        <JarvisInput placeholder="Language" {...form.register(`languages.${i}`)} />
                        {form.watch("languages").length > 1 && (
                          <JarvisButton variant="danger" size="sm" onClick={() => { const l = form.getValues("languages"); form.setValue("languages", l.filter((_: string, j: number) => j !== i)) }}>
                            <Trash2 className="h-3.5 w-3.5" />
                          </JarvisButton>
                        )}
                      </div>
                    ))}
                    <JarvisButton variant="secondary" size="sm" onClick={() => form.setValue("languages", [...form.getValues("languages"), ""])}>
                  ...
                  {eduFields.map((f, i) => (
                    <div key={f.id} className="rounded-lg border border-border p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        {eduFields.length > 1 && <JarvisButton variant="danger" size="sm" onClick={() => removeEdu(i)}><Trash2 className="h-3.5 w-3.5" /></JarvisButton>}
                      </div>
                      <JarvisInput label="Degree" placeholder="BSc, MSc, PhD" {...form.register(`education.${i}.degree`)} />
                      <JarvisInput label="Institution" placeholder="University name" {...form.register(`education.${i}.institution`)} />
                      <div className="grid grid-cols-2 gap-3">
                        <JarvisInput label="Field" placeholder="Physics" {...form.register(`education.${i}.field`)} />
                        <JarvisInput label="Year" placeholder="2020" {...form.register(`education.${i}.year`)} />
                      </div>
                    </div>
                  ))}
                  <JarvisButton variant="secondary" size="sm" onClick={() => addEdu({ degree: "", institution: "", year: "", field: "" })}><Plus className="h-3.5 w-3.5 mr-1" /> Add Education</JarvisButton>
                </div>
              )}
                  ...
                  {expFields.map((f, i) => (
                    <div key={f.id} className="rounded-lg border border-border p-4 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <JarvisButton variant="danger" size="sm" onClick={() => removeExp(i)}><Trash2 className="h-3.5 w-3.5" /></JarvisButton>
                      </div>
                      <JarvisInput label="Title" placeholder="Senior Tutor" {...form.register(`experience.${i}.title`)} />
                      <JarvisInput label="Institution" placeholder="School or org" {...form.register(`experience.${i}.institution`)} />
                      <JarvisInput label="Years" placeholder="3" {...form.register(`experience.${i}.years`)} />
                    </div>
                  ))}
                  <JarvisButton variant="secondary" size="sm" onClick={() => addExp({ title: "", institution: "", years: "", description: "" })}><Plus className="h-3.5 w-3.5 mr-1" /> Add Experience</JarvisButton>
                ...
                <div className="space-y-4">
                  {subFields.map((f, i) => (
                    <div key={f.id} className="flex items-end gap-3">
                      <div className="flex-1"><JarvisInput label={i === 0 ? "Subject" : undefined} placeholder="Mathematics" {...form.register(`subjects.${i}.subject`)} /></div>
                      <div className="w-40">
                        <label className="mb-1.5 block text-sm font-medium text-text-primary">Level</label>
                        <select className="w-full rounded-lg border border-border bg-surface px-3 py-2.5 text-sm text-text-primary" {...form.register(`subjects.${i}.level`)}>
                          {LEVELS.map((l) => <option key={l} value={l}>{l}</option>)}
                        </select>
                      </div>
                      {subFields.length > 1 && <JarvisButton variant="danger" size="sm" onClick={() => removeSub(i)}><Trash2 className="h-3.5 w-3.5" /></JarvisButton>}
                    </div>
                  ))}
                  <JarvisButton variant="secondary" size="sm" onClick={() => addSub({ subject: "", level: "beginner" })}><Plus className="h-3.5 w-3.5 mr-1" /> Add Subject</JarvisButton>
                </div>
              )}
              ...
              {step === 4 && (
                <div className="space-y-4">
                  ...
                  <JarvisInput label="Group (up to 5) ₨/mo" placeholder="5000" {...form.register("group_price_pkr")} />
                  <JarvisInput label="Standard (up to 3) ₨/mo" placeholder="10000" {...form.register("standard_price_pkr")} />
                  <JarvisInput label="Private (1-on-1) ₨/mo" placeholder="20000" {...form.register("private_price_pkr")} />
                </div>
              )}
              ...
              {step === 5 && (
                <div className="space-y-3">
                  ...
                  {DAYS.map((day) => {
                    const sel = form.watch(`availability.${day}`) || []
                    return (
                      <div key={day} className="flex items-center gap-3">
                        <span className="w-10 text-sm font-mono text-text-muted uppercase">{day}</span>
                        <div className="flex flex-wrap gap-2">
                          {SLOTS.map((slot) => (
                            <button key={slot} type="button" onClick={() => toggle(day, slot)}
                              className={`rounded-lg border px-3 py-1.5 text-xs font-medium transition-colors ${sel.includes(slot) ? "border-accent-primary bg-accent-primary/20 text-accent-primary" : "border-border text-text-muted hover:border-accent-primary/50"}`}>
                              {slot}
                            </button>
                          ))}
                        </div>
          ...
          {error && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-4 rounded-lg border border-accent-danger/30 bg-accent-danger/10 p-3 text-sm text-accent-danger">{error}</motion.div>}

          <JarvisDivider className="my-6" />

          <div className="flex items-center justify-between">
            <JarvisButton variant="secondary" onClick={() => setStep(Math.max(0, step - 1))} disabled={step === 0}>
              <ArrowLeft className="h-4 w-4" /> Back
            </JarvisButton>
            <span className="text-xs font-mono text-text-muted">Step {step + 1}/{STEPS.length}</span>
            {step < STEPS.length - 1 ? (
              <JarvisButton variant="primary" onClick={() => setStep(step + 1)} disabled={!canNext()}>
                Next <ArrowRight className="h-4 w-4" />
              </JarvisButton>
            ) : (
              <JarvisButton variant="primary" onClick={form.handleSubmit(onSubmit)} loading={loading}>
