...
export function DashboardSidebar({ role, collapsed, onToggle, mobileOpen, onMobileClose }: Props) {
  ...
  const sidebar = (
    <div className="flex h-full flex-col">
      <div className={cn("flex h-16 items-center border-b border-border px-4", collapsed && "justify-center")}>
        <Link href="/" className="font-display text-lg font-bold bg-gradient-to-r from-accent-primary to-accent-secondary bg-clip-text text-transparent">
          {collapsed ? "H" : "HAYESH"}
        </Link>
      </div>
      ...
      {!collapsed && (
        <div className="px-4 py-3">
          <span className={cn("text-xs font-mono uppercase tracking-widest", roleColors[role])}>{role}</span>
        </div>
      )}
          ...
          return (
            <Link key={item.href} href={item.href} onClick={onMobileClose}
              className={cn("flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all",
                active ? "bg-accent-primary/10 text-accent-primary" : "text-text-muted hover:bg-surface-elevated hover:text-text-primary",
      ...
      <div className="border-t border-border p-2">
        <button onClick={signOut}
          className={cn("flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-text-muted hover:bg-accent-danger/10 hover:text-accent-danger transition-all",
            collapsed && "justify-center px-2")}>
  ...
  return (
    <>
      <motion.aside animate={{ width: collapsed ? 64 : 256 }} transition={{ duration: 0.2 }}
        className="hidden lg:fixed lg:inset-y-0 lg:left-0 lg:z-40 lg:flex lg:flex-col glass-strong">
        {sidebar}
      </motion.aside>
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside initial={{ x: -280 }} animate={{ x: 0 }} exit={{ x: -280 }}
              ...
              <button onClick={onMobileClose} className="absolute right-3 top-3 text-text-muted hover:text-text-primary">
                <X className="h-5 w-5" />
              </button>
              {sidebar}
            </motion.aside>
